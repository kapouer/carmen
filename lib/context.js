...
module.exports = function(geocoder, lon, lat, maxtype, full, search, callback) {
    var context = [];
    ...
    types.forEach(function(type) {
        ...
        q.defer(method, source, lon, lat, full, search);
    });
...
function contextVector(source, lon, lat, full, search, callback) {
    ...
    function query(err, vt) {
        ...
        if (results.length > 1) {
            results.forEach(function(r){
                var butes = r.attributes();
                names = butes._text ? butes._text.split(',') : [];
                names.forEach(function(name){
                    if (!altname && search.join(' ').indexOf(name.toLowerCase()) > -1) {
                        if (name != results[0].attributes()._text) {
                            altname = name;
                        }
                    }
                });
...
function contextGrid(source, lon, lat, full, search, callback) {
    var xyz = sm.xyz([lon, lat, lon, lat], source._geocoder.maxzoom);
    var z = source._geocoder.maxzoom;
    var x = xyz.minX;
    var y = xyz.minY;
